...
function expired(row: { acquired_at?: string; last_heartbeat_at?: string; ttl_seconds?: number }): boolean {
  const ttl = Number(row.ttl_seconds || 0);
  if (!ttl) return false;
  const anchor = Date.parse(row.last_heartbeat_at || row.acquired_at || "");
  return Number.isFinite(anchor) && Date.now() - anchor > ttl * 1000;
}
...
export class RuntimeLeaseStore {
  ...
  constructor(dbPath = process.env.WAH_RUNTIME_SQLITE_PATH || path.join(getStoragePaths().dataDir, "runtime-pool.sqlite")) {
  ...
  gcExpiredLeases(): { profiles: number; tabs: number } {
    ...
    if (this.sqlite) {
      ...
      for (const row of activeProfiles) {
        ...
        if (expired(observed) || (observed.pid && !isPidAlive(observed.pid))) {
      ...
      for (const row of activeTabs) if (expired(row)) { expireTab.run(row.lease_id); this.mirrorTabLease({ ...row, status: "expired" }); tabs++; } else this.mirrorTabLease(row);
      return { profiles, tabs };
    }
    for (const row of this.memory.profile_leases) {
      if (row.status === "active" && (expired(row) || !isPidAlive(row.pid))) { row.status = "expired"; row.last_heartbeat_at = at; profiles++; }
    }
  ...
  activeProfileLease(profileId: string): ProfileLeaseRow | undefined {
    ...
    if (this.sqlite) return this.sqlite.prepare(`SELECT * FROM profile_leases WHERE profile_id=? AND status='active' ORDER BY acquired_at DESC LIMIT 1`).get(profileId);
  ...
  acquireTabLease(profileLeaseId: string, urlMatch: string, ttlSeconds = 300): TabLeaseRow {
    this.gcExpiredLeases();
    const row: TabLeaseRow = { lease_id: leaseId("tab_lease"), profile_lease_id: profileLeaseId, url_match: urlMatch, acquired_at: now(), ttl_seconds: ttlSeconds, status: "active" };
    if (this.sqlite) this.sqlite.prepare(`INSERT INTO tab_leases (lease_id, profile_lease_id, url_match, acquired_at, ttl_seconds, status) VALUES (?, ?, ?, ?, ?, ?)`).run(row.lease_id, row.profile_lease_id, row.url_match, row.acquired_at, row.ttl_seconds, row.status);
