...
# Bucket L2 — Claude incognito + style (2 capability)
...
## cap claude-send-incognito-ext
```bash
node dist/src/cli.js webai:claude:send-prompt --profile claude-9224 --backend extension-assisted-cdp --incognito --prompt "say only OK" --response-timeout-ms 90000 --json
```
- 闭环: `response_text` 非空 AND `chat_url` 含 "incognito=" 子串

## cap claude-send-style-ext
两个 style 各试一次（每次新 chat）：
```bash
node dist/src/cli.js webai:claude:send-prompt --profile claude-9224 --backend extension-assisted-cdp --style "Concise" --prompt "Tell me one sentence about machine learning." --response-timeout-ms 90000 --json
node dist/src/cli.js webai:claude:send-prompt --profile claude-9224 --backend extension-assisted-cdp --style "Explanatory" --prompt "Tell me one sentence about machine learning." --response-timeout-ms 90000 --json
```
- 闭环: 两次都 `completion_detected === true` AND `response_text` 非空 AND 两次输出文本不同（style 应有影响）

如果 Claude 当前账号无 style picker（旧 UI），→ FAIL_CLOSED + UNSUPPORTED_FEATURE，继续。

## 通用 5 步
1-2. CLI + evidence
3. capability-library 追加（status: OK_EXT_BACKEND, mcp_tool: webai_claude_send_prompt, backend: extension-assisted-cdp, source: L2, verified_by: codex-bucket-L2）
4. `examples/workflows/claude-<cap-id>.yaml`
5. 心跳

## 失败 / 兜底 / 退出
2 caps 全败 → L2-blocker.md exit 1。> 25 min exit 2。
