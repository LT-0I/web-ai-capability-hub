...
# Bucket L3 — Claude upload (2 capability)
...
## 前置
确保 /tmp/explore-2026-05-25/fixtures/ 有 fileA.txt / fileB.txt / fileC.txt（C3 已建过）。若无：
```bash
mkdir -p /tmp/explore-2026-05-25/fixtures
echo "alpha doc" > /tmp/explore-2026-05-25/fixtures/fileA.txt
echo "beta doc" > /tmp/explore-2026-05-25/fixtures/fileB.txt
echo "gamma doc" > /tmp/explore-2026-05-25/fixtures/fileC.txt
```

## cap claude-upload-single-ext
```bash
node dist/src/cli.js webai:claude:upload-and-query --profile claude-9224 --backend extension-assisted-cdp --file /tmp/explore-2026-05-25/fixtures/fileA.txt --prompt "Quote the first 3 words of this file verbatim" --response-timeout-ms 120000 --json
```
- 闭环: `response_text` 含 "alpha" AND `attachment_names` 长度 == 1 AND `completion_detected === true`

## cap claude-upload-multi-ext
```bash
node dist/src/cli.js webai:claude:upload-and-query --profile claude-9224 --backend extension-assisted-cdp --file /tmp/explore-2026-05-25/fixtures/fileA.txt --file /tmp/explore-2026-05-25/fixtures/fileB.txt --file /tmp/explore-2026-05-25/fixtures/fileC.txt --prompt "List the unique first word from each file" --response-timeout-ms 180000 --json
```
- 闭环: `response_text` 含 "alpha" AND "beta" AND "gamma" AND `attachment_names` 长度 == 3

注意 Claude 上传上限是 3 文件（contract 已约束）。

## 通用 5 步
1-2. CLI + evidence
3. capability-library 追加（status: OK_EXT_BACKEND, mcp_tool: webai_claude_upload_and_query, backend: extension-assisted-cdp, source: L3, verified_by: codex-bucket-L3）
4. `examples/workflows/claude-<cap-id>.yaml`
5. 心跳

## 失败 / 兜底 / 退出
2 caps 全败 → L3-blocker.md exit 1。> 25 min exit 2。
