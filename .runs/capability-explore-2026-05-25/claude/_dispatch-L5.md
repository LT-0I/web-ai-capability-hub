...
# Bucket L5 — Claude generate_file 代码类 (3 capability: py + md + csv)
...
## cap claude-generate-file-py-ext
```bash
node dist/src/cli.js webai:claude:generate-file --profile claude-9224 --backend extension-assisted-cdp --prompt "Write a Python script that prints 'hello claude probe'. Provide as a downloadable .py file." --expected-extension py --download-dir /tmp/explore-2026-05-25/claude --artifact-class code --json
```
- 闭环: `path` 非空 + 文件存在 + size>32 + 内容含 'hello claude probe'

## cap claude-generate-file-md-ext
```bash
node dist/src/cli.js webai:claude:generate-file --profile claude-9224 --backend extension-assisted-cdp --prompt "Write a markdown README about a project 'ClaudeCapProbe'. Provide as .md file." --expected-extension md --download-dir /tmp/explore-2026-05-25/claude --artifact-class code --json
```
- 闭环: `path` 非空 + 文件存在 + size>64 + 含 'ClaudeCapProbe'

## cap claude-generate-file-csv-ext
```bash
node dist/src/cli.js webai:claude:generate-file --profile claude-9224 --backend extension-assisted-cdp --prompt "Generate a small CSV with header 'name,score' and 3 data rows. Provide as .csv file." --expected-extension csv --download-dir /tmp/explore-2026-05-25/claude --artifact-class code --json
```
- 闭环: `path` 非空 + 文件存在 + size>32 + 含 'name,score'

## 通用 5 步
1-2. CLI + evidence
3. capability-library 追加（status: OK_EXT_BACKEND, mcp_tool: webai_claude_generate_file, backend: extension-assisted-cdp, source: L5, verified_by: codex-bucket-L5, artifact: {...}）
4. `examples/workflows/claude-<cap-id>.yaml`
5. 心跳

## 失败 / 兜底 / 退出
3 caps 全败 → L5-blocker.md exit 1。> 25 min exit 2。
