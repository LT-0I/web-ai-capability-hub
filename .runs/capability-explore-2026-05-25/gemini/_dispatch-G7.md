...
# Bucket G7 — Gemini canvas_to_docs (1 capability)
...
## cap gemini-canvas-to-docs-mgr
```bash
node dist/src/cli.js webai:gemini:canvas-to-docs --profile gemini-9225 --tab-url-contains "<chat-id>" --prompt "Generate a 200-word essay about deep learning, then export to Google Docs" --title "Capability Probe Doc" --response-timeout-ms 300000 --json
```
- 闭环: 返回 `docs_url` 非空 AND URL 含 `docs.google.com` AND 可 curl GET 拿到 200 (or redirect 30x to login page是预期)

## 通用 5 步
1-2. CLI + evidence
3. capability-library 追加（status: OK_MANAGED_CDP_ONLY, mcp_tool: webai_gemini_canvas_to_docs, backend: managed-cdp, source: G7, verified_by: codex-bucket-G7, artifact: {type: text/html, public_url: <docs_url>}）
4. `examples/workflows/gemini-canvas-to-docs.yaml`
5. 心跳

## 失败 / 兜底 / 退出
账号无 Docs 权限 → fail-closed UNSUPPORTED_FEATURE。> 25 min exit 2。
