...
# Bucket G4 — Gemini generate_video (Veo) (1 capability)
...
## cap gemini-generate-video-ext
```bash
node dist/src/cli.js webai:gemini:generate-video --profile gemini-9225 --backend extension-assisted-cdp --reuse-conversation --tab-url-contains "<chat-id>" --prompt "A 5-second loop of a serene mountain landscape with soft camera pan" --download-dir /tmp/explore-2026-05-25/gemini --response-timeout-ms 600000 --json
```
- 闭环: `path` 非空 + 文件存在 + 文件名 `.mp4` 结尾 + `size_bytes > 16384`

**Veo 配额风险**: 如果 errorCode 含 `EXTERNAL_QUOTA_EXHAUSTED` 或 response 含 "quota" / "limit"：
- 标 `quota-limited` + status: `FAIL_CLOSED_QUOTA`
- **不切换 profile**，直接 fail-closed 写 library 然后 exit 0（桶完成）

## 通用 5 步
1-2. CLI + evidence
3. capability-library 追加（status: OK_EXT_BACKEND 或 FAIL_CLOSED_QUOTA, mcp_tool: webai_gemini_generate_video, backend: extension-assisted-cdp, source: G4, verified_by: codex-bucket-G4, artifact: {type: video/mp4, relative_path: <file>} 仅成功时）
4. `examples/workflows/gemini-generate-video-ext.yaml`（即使 quota 也要写 recipe，标 `expected_failure: EXTERNAL_QUOTA_EXHAUSTED`）
5. 心跳

## 失败 / 兜底 / 退出
配额耗尽 = 预期失败，桶正常 exit 0。其他失败 1 次重试，仍败 → G4-blocker.md exit 1。视频生成可能 10+ 分钟，response-timeout 给 600000 ms。> 25 min exit 2。
