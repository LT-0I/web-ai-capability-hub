...
# Bucket G9 — Gemini Gems 子产品 (1 capability)
...
- backend: managed-cdp（Gems 还没 ext opt-in）
- 硬性: `--reuse-conversation`

## cap gemini-gems-converse-mgr
- 通过 URL 直接进入一个 Gems 实例（如 https://gemini.google.com/gem/<gem-id>），用 `--url` 指定，然后 send-prompt
- 优先用账号已 pin 的 Gem；若无，用任一公开 Gem
```bash
# 先列已存在的 Gem tabs 或选一个 public Gem
node dist/src/cli.js webai:gemini:send-prompt --profile gemini-9225 --url "https://gemini.google.com/gem/coding-partner" --prompt "Say only OK" --response-timeout-ms 90000 --json
```
- 闭环: `response_text` 非空 + `chat_url` 含 `/gem/`

如账号无 Gems 权限 / Gem URL 不可达 → fail-closed UNSUPPORTED_FEATURE。

## 通用 5 步
1-2. CLI + evidence
3. capability-library 追加（status: OK_MANAGED_CDP_ONLY 或 FAIL_CLOSED_UNSUPPORTED, mcp_tool: webai_gemini_send_prompt（with gem url）, backend: managed-cdp, source: G9, verified_by: codex-bucket-G9）
4. `examples/workflows/gemini-gems-converse.yaml`
5. 心跳

## 失败 / 兜底 / 退出
失败 → G9-blocker.md exit 1。> 25 min exit 2。
