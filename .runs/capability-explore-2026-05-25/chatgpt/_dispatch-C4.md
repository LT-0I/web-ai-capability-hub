...
# Bucket C4 — ChatGPT generate_file 文档类 (2 capability: docx + pptx)
...
## cap chatgpt-generate-file-docx-ext
```bash
node dist/src/cli.js webai:chatgpt:generate-file --profile chatgpt --backend extension-assisted-cdp --prompt "Write a one-page Word document titled 'Capability Probe Docx' with 3 short paragraphs about quantum computing. Then provide it as a downloadable .docx file." --expected-extension docx --download-dir /tmp/explore-2026-05-25/chatgpt --artifact-class document --json
```
- 闭环判据: `path` 非空 AND 文件存在 AND `size_bytes > 4096` AND 文件名以 `.docx` 结尾

## cap chatgpt-generate-file-pptx-ext
```bash
node dist/src/cli.js webai:chatgpt:generate-file --profile chatgpt --backend extension-assisted-cdp --prompt "Create a 5-slide pptx presentation about renewable energy basics, with one title slide and 4 content slides. Deliver as a downloadable .pptx." --expected-extension pptx --download-dir /tmp/explore-2026-05-25/chatgpt --artifact-class document --json
```
- 闭环判据: `path` 非空 AND 文件存在 AND `size_bytes > 8192` AND 文件名以 `.pptx` 结尾

## 通用 5 步
1. 跑 CLI，捕获 JSON
2. evidence → `.runs/capability-explore-2026-05-25/chatgpt/<cap-id>.json`
3. 闭环 → capability-library features 追加（status: OK_EXT_BACKEND, mcp_tool: webai_chatgpt_generate_file, backend: extension-assisted-cdp, source: capability-explore-2026-05-25/C4, verified_by: codex-bucket-C4, artifact: {type: <mime>, relative_path: <file>}）
4. 闭环 → `examples/workflows/chatgpt-<cap-id>.yaml`
5. 心跳：`<ISO> C4/<cap-id> <ok|fail-closed> <wall_ms> <evidence>`

## 失败处理
单 cap 2 次；仍败 → FAIL_CLOSED_EXT_BACKEND + errorCode + cause，继续。2 caps 全败 → 桶 blocker C4-blocker.md exit 1。

## 兜底 / 退出
同 C2/C3。> 25 min exit 2。
