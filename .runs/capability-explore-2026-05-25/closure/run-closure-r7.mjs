...
const RUN_ROOT = path.join(ROOT, '.runs/capability-explore-2026-05-25/closure-r7');
const RESULTS_JSONL = path.join(RUN_ROOT, 'closure-results.jsonl');
const REPORT_PATH = path.join(ROOT, '.runs/capability-explore-2026-05-25/closure-r7-report.md');
...
function closureMatches(feature, parsed) {
  ...
  switch (id) {
    ...
    case 'chatgpt-deep-research-ext': return { ok: nonEmptyKey(parsed, 'task_id') || pathWithMin(parsed, 4096) || countFrameworkNames(text || all) >= 3, cause: 'deep research task_id/report/framework gate failed' };
    ...
    case 'claude-design-present-mgr': return { ok: /\/serve\/|\?file=/.test(urlText) && (nonEmptyKey(parsed, 'presentUrl') || nonEmptyKey(parsed, 'present_url') || nonEmptyKey(parsed, 'publicUrl') || nonEmptyKey(parsed, 'public_url')), cause: 'presentUrl/publicUrl gate failed' };
