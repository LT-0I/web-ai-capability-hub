...
const RUN_ROOT = path.join(ROOT, '.runs/capability-explore-2026-05-25/closure-r3');
const RESULTS_JSONL = path.join(RUN_ROOT, 'closure-results.jsonl');
const REPORT_PATH = path.join(ROOT, '.runs/capability-explore-2026-05-25/closure-r3-report.md');
...
function closureMatches(feature, parsed) {
  ...
  switch (id) {
    ...
    case 'claude-design-create-project-mgr': return { ok: nonEmptyKey(parsed, 'project_id') && nonEmptyKey(parsed, 'project_url'), cause: 'project_id/project_url gate failed' };
    ...
    case 'claude-send-style-ext': {
      ...
      return { ok: responses.length >= 2 && responses[0] !== responses[1] && completion, cause: 'style dual-response gate failed' };
    }
    ...
    case 'gemini-workspace-mgr': return { ok: Array.isArray(workspaces) || hasCount, cause: 'workspace array/count gate failed' };
    default:
