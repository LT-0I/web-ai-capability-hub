...
test("phase8 contract guard reflects package, contract, and 8-lock counts", () => {
  ...
  const golden = JSON.parse(fs.readFileSync(path.resolve(process.cwd(), "tests/golden/listMcpTools.216.json"), "utf8"));
  ...
  assert.equal(contract.commands.filter((command: any) => String(command.mcp_name || "").startsWith("webai_")).length, 61);
