...
test("p3: consumer-contract.json contract_version stays consumer-contract-1.7.1 (P3 does not bump contract)", () => {
...
test("p3: scripts/verify-contract-version.ts EXPECTED_CONTRACT_VERSION === 'consumer-contract-1.7.1'", () => {
  assert.match(VERIFY, /EXPECTED_CONTRACT_VERSION\s*=\s*"consumer-contract-1\.7\.1"/);
});
