  ...
  try {
    ...
    await assert.rejects(
      ...
      (error: any) => {
        ...
        assert.deepEqual(error.evidence, { timeoutMs: 1000, bufferedBytes: 0, "bodies.size": 0, responseReceivedSeen: 0, imageGatePassed: 0, streamArmAttempts: 0, eagerRawBodyEntries: 0, resourceTreeMatches: 0, resourceContentRetrieved: 0 });
        assert.deepEqual(fs.readdirSync(dir), []);
        return true;
      }
    );
