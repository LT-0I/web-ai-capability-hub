...
test("Claude deep_research RPC decodeClaudeRpcSseEnvelope decodes captured start fixture assistant text and model", () => {
  const fixturePath = path.join(CAPTURE_ROOT, "webai_claude_deep_research--start/response-stream.txt");
  const fixture = fs.readFileSync(fixturePath, "utf8");
  const decoded = decodeClaudeRpcSseEnvelope(fixture);
  assert.equal(decoded.modelUsed, "claude-sonnet-4-6");
  assert.equal(decoded.messageUuid, "019e69f1-a8c4-7dc1-9d32-13aae7f4b052");
  assert.match(decoded.responseText, /internal system ping/);
  assert.match(decoded.responseText, /What would you like me to research\?/);
});
...
test("Claude deep_research RPC start returns queued DOM-compatible task envelope using captured SSE fixture", async () => {
  const fixturePath = path.join(CAPTURE_ROOT, "webai_claude_deep_research--start/response-stream.txt");
  ...
  const fetchRpc: ClaudeDeepResearchRpcFetch = async (request) => {
    ...
    return { status: 200, contentType: "text/event-stream; charset=utf-8", text: stream, elapsedMs: 73 };
  };
  ...
  const result: any = await webAiClaudeDeepResearchRpcWithFetch(
    ...
    fetchRpc,
    { orgId: ORG_UUID, conversationId: CONVERSATION_UUID, taskId: "claude_research_captured" }
  );

  assert.equal(body.create_conversation_params.compass_mode, "advanced");
  assert.equal(result.errorCode, null);
  assert.equal(result.task_id, "claude_research_captured");
  assert.equal(result.status, "queued");
  assert.equal(result.chat_url, `https://claude.ai/chat/${CONVERSATION_UUID}`);
  assert.equal(result.wait_ms, 73);
