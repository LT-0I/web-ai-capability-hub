...
`webai:chatgpt:select-model` / `webai_chatgpt_select_model`, `webai:claude:select-model` / `webai_claude_select_model`, and `webai:gemini:select-model` / `webai_gemini_select_model` select provider model/thinking controls without sending a prompt. Required args: `profile`. ChatGPT accepts optional human-readable `model` picker labels and `thinking_level` (`auto`, `extended`); `extended` maps to the Thinking tier. Claude accepts optional human-readable `model` picker labels and `thinking_level` (`auto`, `extended`, `off`, `low`, `medium`, `high`, `xhigh`, `max`); the RPC default writes Claude's `/api/organizations/<org>/model_selector_state/chat` selector state, preserving remembered thinking on model-only changes and using effort levels for Opus/Sonnet where supported. Gemini keeps `model` (`3.1-flash-lite`, `3.5-flash`, `3.1-pro`) and `thinking_level` (`standard`, `extended`). Output shape: `ok`, `selected_model`, `selected_thinking_level`, `errorCode`. Error codes: `ELEMENT_NOT_FOUND`, `COMMAND_TIMEOUT`, `INVALID_ARGS`, `MODEL_SELECTION_DRIFT`.
