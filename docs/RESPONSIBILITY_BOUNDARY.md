...
| **R2** Stable structured return (#8) | **PASS on the structured-success semantic, with one open contract question** | **R2 — PASS on the structured-success semantic (`errorCode===null` + populated governed `path`), with one honest open item:** the hub does not emit a literal `ok` boolean; success is signalled by `errorCode:null` + a populated `path` (mapped by `artifactClickResultToSafeOutput`, `tools.ts:1401-1413`). The #8/#8-R2 empty-`path` defect is fully closed at `d2cc581` (no empty-success path remains; 399/399). **Open contract question to the consumer:** do you accept `errorCode===null && path!=""` as the canonical success predicate (then we ratify it verbatim in the boundary doc), OR do you require a literal `ok:true`/`ok:false` field? If the latter, the hub will add `ok` as an **announced contract surface change** — a deliberate §2.4 round-trip (`configs/consumer-contract.json` + `docs/CONSUMER_CONTRACT.md` + `tests/consumerContract.test.ts`) with a `contract_version` bump (`consumer-contract-1.5.0`→`1.6.0`) and a fresh `Fixed in <sha>`. We will not silently redefine your requirement nor silently under-ship the field. |
| **R3** Deterministic async terminal contract | **PASS** after docs clarification | Gemini video has a reaper and a documented poll-until-terminal contract. Music and codex task-status calls are synchronous in-page reads with no orphanable hub record. `docs/CONSUMER_CONTRACT.md` now records the music terminal set `{complete, error}` (non-terminal `generating`), the codex terminal set `{complete}` / non-terminal `{running}` / `INVALID_ARGS` on unknown, and the universal `withMcpToolDeadline` bound: default `180000` ms, max `600000` ms, producing `COMMAND_TIMEOUT`. No hub poll loop can run forever. |
