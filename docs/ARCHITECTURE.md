...
4. **Capability Database Layer**: `CapabilityDatabase` initializes SQLite schema/migrations when `better-sqlite3` is installed and otherwise uses a local JSON fallback with the same repository API for constrained environments. Runtime auto-extracted capabilities live in `capabilities`; integration/exploration governance lives in `integration_registry`, seeded from `docs/capability-library.json` via `capability:library:import` — one store, two tables, two views.
