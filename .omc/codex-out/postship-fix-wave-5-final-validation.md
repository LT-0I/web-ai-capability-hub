# Post-ship fix wave 5 final validation

Generated: 2026-05-26. Source regression artifact: `.runs/postship-fix-wave-5/regression-summary.md`.

## 1. Per-yaml regression table (63 rows)
...
| Cluster | Baseline remaining | Post-sweep remaining | Now passing | Remaining failures |
|---|---:|---:|---:|---|
| Bridge race | 6 | 3 | 3/6 | `gemini-canvas-edit`, `gemini-canvas-to-docs`, `chatgpt-codex-submit-task` |
| Gemini selectors | 12 | 4 | 8/12 | `gemini-send-web-search`, `gemini-veo-quota-error`, `gemini-generate-video`, `gemini-music-generate-chain` |
| ChatGPT selectors | 8 | 2 | 6/8 | `chatgpt-canvas-create-export`, `chatgpt-send-web-search` |
| Completion detectors | 13 | 8 | 5/13 | 5 ChatGPT generate-file timeouts, Claude pptx download timeout, Claude Design generate/present extension gates |
| One-offs | 5 | 3 | 2/5 | `claude-design-present`, `gemini-image-draft`, `research-database-search-dry-run` |

## 3. Net PASS rate improvement

- Baseline: 18/63 PASS (28.6%).
- Post-sweep: 42/63 PASS (66.7%).
- Net improvement: +24 PASS, +38.1 percentage points.
- Deferred due ChatGPT rate limit: 0.
...
```json
{
  "package": "2.1.0",
  "package_lock": "2.1.0",
  "contract_version": "consumer-contract-2.1.0",
  "commands": 232,
  "error_codes": 40,
  "webai": 81,
  "research": 121,
  "wah": 8,
  "golden": "listMcpTools.236.json"
}
```

`git diff -- src configs package.json package-lock.json tests/golden/listMcpTools.236.json --exit-code --stat` produced no output and exit 0.

## 5. git log -1 --oneline + push

The exact final commit hash is reported after commit creation and push in the final assistant response; embedding a commit's own hash inside this committed artifact would change the commit hash. Planned commit title:

```text
webai: post-ship fix wave 5 — final 63-yaml regression re-baseline + docs sweep
```

## 6. Auto-close grep

Commit message auto-close grep target:

```bash
grep -Eio '(close[sd]?|fix(e[sd])?|resolve[sd]?) #[0-9]+' /tmp/postship-fix-wave-5-commit-message.txt
```
...
- Full serialized 63-yaml regression: completed 63/63, PASS 42, FAIL 21, deferred-rate-limit 0.
- Scope: no `src/` production-driver changes, no package bump, no contract-version bump.
